...
// 分区快排
public class T005_PartitionAndQuickSortByStackV3 {
    ...
    private static void process(int[] arr, int l, int r){
        ...
        // 下面的代码替代了递归
        Queue<StackTmp> stack = new LinkedList<>();
        stack.offer(new StackTmp(l, m[0] -1));
        stack.offer(new StackTmp(m[0] + 1, r));

        while (!stack.isEmpty()) {
            StackTmp pop = stack.poll();
            if(pop.l < pop.r) {
                swap(arr, pop.l + (int) (Math.random() * (pop.r - pop.l + 1)), pop.r);
                int[] range = partition(arr, pop.l, pop.r);
                stack.offer(new StackTmp(pop.l, range[0] - 1));
                stack.offer(new StackTmp(range[1] + 1, pop.r));
            }
        }
    }
